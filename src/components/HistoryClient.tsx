...
export default function HistoryClient({ initialHistory }: HistoryClientProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 dark:bg-gray-950 transition-colors duration-300">
      <div className="max-w-3xl mx-auto bg-white shadow-lg rounded-lg p-8 dark:bg-gray-800 dark:text-gray-100 transition-colors duration-300">
        <h1 className="text-3xl font-bold text-gray-900 mb-6 dark:text-gray-50">Your Search History</h1>
        ...
        {history.length === 0 ? (
          ...
          <ul className="space-y-4 mb-6">
            {history.map((entry) => (
              <li
                key={entry.id}
                className="flex justify-between items-center bg-gray-100 p-4 rounded-md shadow-sm hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 transition-colors duration-300"
              >
                <Link
                  href={`/food/${encodeURIComponent(entry.foodName)}`}
                  className="text-blue-600 hover:underline text-xl font-medium dark:text-blue-400 flex-grow"
                >
                  {entry.foodName}
                </Link>
                <span className="text-gray-500 text-sm dark:text-gray-400 mr-4">
